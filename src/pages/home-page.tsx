          ...
          <div className="relative">
            {loading ? (
              ...
              <div className="relative">
                ...
                {/* Smart indicator dots with ellipsis for many pages */}
                {galleryImages.length > imagesPerSet && (
                  <div className="flex justify-center mt-6 space-x-1">
                    {(() => {
                      ...
                      if (totalSets <= maxDots) {
                        // Show all dots if total sets is less than max
                        ...
                        dots.push(
                          <button
                            ...
                            onClick={() => setCurrentGallerySet(0)}
                          >
                            <span className="sr-only">Go to image set 1</span>
                          </button>
                        );

                        // Show ellipsis if current set is not near beginning
                        if (currentGallerySet > 2) {
                          dots.push(
                            <span
                              key="ellipsis1"
                        ...
                        // Show current set and neighbors
                        for (
                          let i = Math.max(1, currentGallerySet - 1);
                          i <= Math.min(totalSets - 2, currentGallerySet + 1);
                          i++
                        ) {
                          dots.push(
                            <button
                              ...
                              onClick={() => setCurrentGallerySet(i)}
                            >
                              <span className="sr-only">
                                Go to image set {i + 1}
                              </span>
                        ...
                        // Show ellipsis if current set is not near end
                        if (currentGallerySet < totalSets - 3) {
                          dots.push(
                            <span
                              key="ellipsis2"
                              className="text-muted-foreground"
                        ...
                        dots.push(
                          <button
                            ...
                            className={cn(
                              "w-3 h-3 rounded-full transition-colors touch-friendly",
                              totalSets - 1 === currentGallerySet
                                ? "bg-primary"
                                : "bg-muted-foreground/30"
                            )}
                            onClick={() => setCurrentGallerySet(totalSets - 1)}
                          >
                            <span className="sr-only">
                              Go to image set {totalSets}
                            </span>
                          </button>
                        );
